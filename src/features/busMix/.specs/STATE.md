...
- `fader-knob-skeuomorphic-refresh`: planned on 2026-05-25. Goal is to restyle the BusMix `VerticalFader` thumb/cap as an off-white, rounded, physical skeuomorphic fader knob with recessed grooves, center calibration line, bevel/volume, and projected shadow. This is visual-only and must preserve thumb-only gesture behavior, fader math, meters, OSC, and remote-sync rollback work.
